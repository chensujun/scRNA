...
library(gdata);
library(plyr);
setwd("/.mounts/labs/cpcgene/private/projects/rnaseq_landscape/scRNA/scran/annotate_type");
source('~/svn/singleCell/myfunctions/run_qusage.R');
conf <- read.config.file('~/svn/singleCell/master_config_scRNA.R');
name <- 'all';
#name <- 'all_20k';
csce <- readRDS(conf$scran_all);
csce$type_ori <- csce$type;
#### using signatures from normal prostate scRNA-seq to differentiate stroma from epithelia
myref <- read.xls('data/1-s2.0-S2211124718318771-mmc2.xlsx', as.is = TRUE);
...
saveRDS(myresult, file = generate.filename('annotate_type', 'result_scnorm', 'rds'));
write.csv(data.frame(csce@colData), generate.filename('annotate_type', 'colData_scnorm', 'csv'));
...
save(myresult, file = generate.filename('annotate_type', 'result_scnorm_epi', 'rds'));
#####
...
write.csv(data.frame(csce@colData), generate.filename('annotate_type', 'colData_manual', 'csv'));
saveRDS(csce, file = paste0('objects/', generate.filename(name, 'annotated', 'rds')));
newcolData <- csce@colData;
saveRDS(newcolData, file = generate.filename('annotate_type', 'colData_manual', 'rds'));
