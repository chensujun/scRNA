# command in linux 
# Rscript35 km_sig_Glinsky.R sb mylist.txt mean
...
load('./data/Glinsky.RData');
mydata <- exp2
# mydata=log2(mydata+1)
clinical <- sur2
colnames(clinical)=c("bcr", "time_to_bcr")
clinical$"time_to_bcr"=clinical$"time_to_bcr"/12
# str(clinical)
...
pdf(generate.filename('KM_Glinsky', paste0(mymethod, '_', gsub('\\..*', '', mylist)), 'pdf'));
myplot;
dev.off();
