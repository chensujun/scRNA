...
run_qusage <- function(csce, nm = 'clusters', gs){
	# Run QuSAGE for each of the clusters for given geneset(s)
	#Inputs:
	#csce = a SCE object from scran
	#nm = name of the run
	...
	ds <- min(table(csce$cluster));
	number.clusters <- length(unique(csce$cluster))
	cell.sample <- NULL
	for (i in 1:number.clusters){
	cell <- colnames(csce[, csce$cluster==i])
	...
	mydata <- logcounts(csce[, colnames(csce)%in%cell.sample])
	labels <- paste0('Cluster_', csce[, colnames(csce)%in%cell.sample]$cluster)
	...
	rownames(results.clust.id) <- NULL
  	save(list=ls(), file = paste0(Sys.Date(), '_', nm, '_qusage_results_', length(gs), '.rda'));
